...
def predict_frame_uni(image1, image2, k_width, ac_block):
    ...
    i2[:, :, 1] = I1[:, :, 1]
    i2[:, :, 0] = I2[:, :, 1]
    mvs = motion_est2(i[:, :, 0], i[:, :, 1], b, max_motion)
    
    c = get_c_m(i, Q, int(ac_block/2), mvs, b)
    C = get_C_m(i, Q, int(ac_block/2), mvs, b)
    A = estimate_coefficients_motion2(c, C)
    #A_star = diag_A(A, mvs, max_motion, k_width, b)

    c2 = get_c_m(i2, Q, int(ac_block/2), -mvs, b)
    C2 = get_C_m(i2, Q, int(ac_block/2), -mvs, b)
    A2 = estimate_coefficients_motion2(c2, C2)
    #A2_star = diag_A(A2, -mvs, max_motion, k_width, b)

    # i_t = 217
    # j_t = 159
    # plt.figure()
    # plt.imshow(A_star[i_t, j_t], cmap='seismic', interpolation='nearest', norm=MidPointNorm(midpoint=0))
    # plt.title("A star")

    # k_w2 = int(2 * max_motion) + int(k_width)
    # cen = int(k_w2 / 2)
    # A_test = np.zeros((k_w2, k_w2))
    # k_off = int(k_width / 2)

    # A_test[cen - k_off + mvs[i_t + b, j_t + b, 0] : cen + k_off + 1 + mvs[i_t + b, j_t + b, 0], \
    #                cen - k_off + mvs[i_t + b, j_t + b, 1] : cen + k_off + 1 + mvs[i_t + b, j_t + b, 1]] = A[i_t, j_t].reshape((k_width, k_width))
    # plt.figure()
    # plt.imshow(A_test,  cmap='seismic', interpolation='nearest', norm=MidPointNorm(midpoint=0))
    # plt.title("A shifted")
    # plt.show()
