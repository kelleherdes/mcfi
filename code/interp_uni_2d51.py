...
def simple_kernel2(image1, image2, image3, x, k_width, ac_block, Q):
    k_size = k_width ** 2
    off = int(ac_block / 2)
    C = np.zeros((2 * k_size, 2 * k_size))
    c = np.zeros(2 * k_size)
    for i in range(0, k_size):
        c[i] = np.sum(image2[x[0]           - off : x[0]           + off + 1, x[1]           - off : x[1]           + off + 1] * \
               image1[x[0] + Q[i, 0] - off : x[0] + Q[i, 0] + off + 1, x[1] + Q[i, 1] - off : x[1] + Q[i, 1] + off + 1])
        c[i + k_size] = np.sum(image2[x[0]           - off : x[0]           + off + 1, x[1]           - off : x[1]           + off + 1] * \
               image3[x[0] + Q[i, 0] - off : x[0] + Q[i, 0] + off + 1, x[1] + Q[i, 1] - off : x[1] + Q[i, 1] + off + 1])

        for j in range(0, k_size):
            C[i, j] = np.sum(image1[x[0] + Q[i, 0] - off : x[0] + Q[i, 0] + off + 1, x[1] + Q[i, 1] - off : x[1] + Q[i, 1] + off + 1] * \
                image1[x[0] + Q[j, 0] - off : x[0] + Q[j, 0] + off + 1, x[1] + Q[j, 1] - off : x[1] + Q[j, 1] + off + 1])
            C[i + k_width, j + k_width] = np.sum(image1[x[0] + Q[i, 0] - off : x[0] + Q[i, 0] + off + 1, x[1] + Q[i, 1] - off : x[1] + Q[i, 1] + off + 1] * \
                image1[x[0] + Q[j, 0] - off : x[0] + Q[j, 0] + off + 1, x[1] + Q[j, 1] - off : x[1] + Q[j, 1] + off + 1])
            C[i, j + k_width] = C[j, i + k_width] = np.sum(image3[x[0] + Q[i, 0] - off : x[0] + Q[i, 0] + off + 1, x[1] + Q[i, 1] - off : x[1] + Q[i, 1] + off + 1] * \
                image1[x[0] + Q[j, 0] - off : x[0] + Q[j, 0] + off + 1, x[1] + Q[j, 1] - off : x[1] + Q[j, 1] + off + 1])
