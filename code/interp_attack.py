...
from motion import motion_pyramid
import math
...
def predict_frame_uni(image1, image2, k_width, ac_block):
    ...
    im1 = np.random.randint(0, 255, size=im1.shape).astype(np.uint8)
    im2 = np.random.randint(0, 255, size=im1.shape).astype(np.uint8)
    
    I1 = cv2.copyMakeBorder(cv2.cvtColor(im1, cv2.COLOR_BGR2YUV), b, b, b, b, cv2.BORDER_REFLECT).astype(np.int32)
    I2 = cv2.copyMakeBorder(cv2.cvtColor(im2, cv2.COLOR_BGR2YUV), b, b, b, b, cv2.BORDER_REFLECT).astype(np.int32)

    Q = generate_Q_auto(k_width)

    i = np.zeros((I1.shape[0], I1.shape[1], 2), dtype = np.int32)
    i[:, :, 0] = I1[:, :, 0]
    i[:, :, 1] = I2[:, :, 0]

    i2 = np.zeros((I1.shape[0], I1.shape[1], 2), dtype = np.int32)
    i2[:, :, 1] = I1[:, :, 0]
    i2[:, :, 0] = I2[:, :, 0]
    mvs = motion_pyramid(np.copy(i[:, :, 0]), np.copy(i[:, :, 1]), b, max_motion)
    
    c = get_c_m(i, Q, int(ac_block/2), mvs, b)
    C = get_C_m(i, Q, int(ac_block/2), mvs, b)
    A = estimate_coefficients_motion2(c, C)
    
    c2 = get_c_m(i2, Q, int(ac_block/2), -mvs, b)
    C2 = get_C_m(i2, Q, int(ac_block/2), -mvs, b)
    A2 = estimate_coefficients_motion2(c2, C2)
    
    predicted = estimate_frame(I1, I2, A, A2, b, mvs, k_width)
    predicted[predicted > 255] = 255
    predicted[predicted < 0] = 0

    predicted = cv2.cvtColor(predicted.astype(np.uint8), cv2.COLOR_YUV2BGR).astype(np.uint8)
    return predicted
