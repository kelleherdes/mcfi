...
@cuda.jit
def cuda_c_m(I, Q, ac_off, mvs, b, c):
    y, x = cuda.grid(2)
    r1 = 0
    ...
    for k in range(0, Q.shape[0]):
        for i in range(-ac_off, ac_off + 1):
            for j in range(-ac_off, ac_off + 1):
                ...
                #i and j are the indices for the block site
                r1 += I[y + i + Q[k, 0] + mvs[y, x, 0], x + j + Q[k, 1] + mvs[y, x, 1], 0] * I[y + i - mvs[y, x, 0], x + j - mvs[y, x, 1], 1] 
        c[y - b, x - b, k]  = r1  
        r1 = 0
        ...
        for l in range(0, Q.shape[0]):
            for i in range(-ac_off, ac_off + 1):
                for j in range(-ac_off, ac_off + 1):
                    #y and x are the indices of the picture
                    #i and j are the indices for the block site
                    r += I[y + i + Q[l, 0] + mvs[y, x, 0], x + j + Q[l, 1] + mvs[y, x, 1], 0] * \
                         I[y + i + Q[k, 0] + mvs[y, x, 0], x + j + Q[k, 1] + mvs[y, x, 1], 0] 
            C[y - b, x - b, k, l] = C[y - b, x - b, l, k] = r
            r = 0

def get_c_m(I, Q, ac_off, mvs, b):
    TPB = (16, 16)
    ...
    cuda_c_m[BPG, TPB](I, Q, ac_off, mvs, b, c)
    return c
        ...
        for l in range(0, Q.shape[0]):
            for i in range(-ac_off, ac_off + 1):
                for j in range(-ac_off, ac_off + 1):
                    #y and x are the indices of the picture
                    #i and j are the indices for the block site
                    r += I[y + i + Q[l, 0], x + j + Q[l, 1], 0] * \
                         I[y + i + Q[k, 0], x + j + Q[k, 1], 0] 
            C[y - b, x - b, k, l] = C[y - b, x - b, l, k] = r
            r = 0
